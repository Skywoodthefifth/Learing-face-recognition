...
def predict(X_img, MLP_clf=None, knn_clf=None, model_path_MLP=None, model_path_knn=None, distance_threshold=0.6):

    if MLP_clf is None and model_path_MLP is None:
        raise Exception(
            "Must supply MLP classifier either thourgh MLP_clf or model_path")

    # Load a trained MLP model (if one was passed in)
    if MLP_clf is None:
        with open(model_path_MLP, 'rb') as f:
            MLP_clf = pickle.load(f)

    if knn_clf is None and model_path_knn is None:
        raise Exception(
    ...
    # Use the MLP model to find the best matches for the test face
    closest_distances = knn_clf.kneighbors(faces_encodings, n_neighbors=1)
    are_matches = [closest_distances[0][i][0] <=
                   distance_threshold for i in range(len(X_face_locations))]

    # Predict classes and remove classifications that aren't within the threshold
    return [(pred, loc) if rec else ("Unknown", loc) for pred, loc, rec in zip(MLP_clf.predict(faces_encodings), X_face_locations, are_matches)]
...
def process(worker_id, read_frame_list, write_frame_list, Global, worker_num):
    while not Global.is_exit:
        ...
        predictions = predict(
            rgb_frame, MLP_clf=Global.MLP_clf, knn_clf=Global.knn_clf, distance_threshold=0.4)
...
if __name__ == '__main__':
    ...
    with open("trained_MLP_model.clf", 'rb') as f:
        Global.MLP_clf = pickle.load(f)
    with open("trained_knn_model.clf", 'rb') as f:
        Global.knn_clf = pickle.load(f)
